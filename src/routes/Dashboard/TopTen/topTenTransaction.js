...
import topTenTransactionData from "../../../components/DataDummy/topTenTransaction.json";

function TopTenTransaction() {
  const [topTenTransaction, setTopTenTransaction] = useState(
    topTenTransactionData
  );
  ...
  return (
    <div>
      {/* {topTenTransaction} */}
      <h2 className="top-title">Top Ten Transaction</h2>
      <ol>
        {dataTopTenTransaction?.slice(0, visible).map((row, i) => (
          <h3>
            ...
            <div style={{ width: "40%", float: "left" }}>
              <h3 className="top-ten-nominal">{row.nominal_Transaction}</h3>
            </div>
          </h3>
        ))}
      </ol>

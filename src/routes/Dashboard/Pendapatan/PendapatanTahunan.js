...
import valuesTahunanThisYearData from "../../../components/DataDummy/valuesTahunanThisYear.json";
import valuesTahunanLastYearData from "../../../components/DataDummy/valuesTahunanLastYear.json";
...
const PendapatanTahunan = (props) => {
  ...
  const [earningThisYear, setEarningThisYear] = useState(
    valuesTahunanThisYearData[0][0].stringValue
  );
  const [earningLastYear, setEarningLastYear] = useState(
    valuesTahunanLastYearData[0][0].stringValue
  );

  const [taxThisYear, setTaxThisYear] = useState(
    valuesTahunanThisYearData[0][1].stringValue
  );
  const [taxLastYear, setTaxLastYear] = useState(
    valuesTahunanLastYearData[0][1].stringValue
  );

  const [nettThisYear, setNettThisYear] = useState(
    valuesTahunanThisYearData[0][2].stringValue
  );
  const [nettLastYear, setNettLastYear] = useState(
    valuesTahunanLastYearData[0][2].stringValue
  );
  ...
  useEffect(() => {
    setMoneyThisYear(
      earningThisYear === undefined ? 0 : Number(earningThisYear)
    );
  }, [earningThisYear]);

  useEffect(() => {
    setMoneyLastYear(
      earningLastYear === undefined ? 0 : Number(earningLastYear)
    );
  }, [earningLastYear]);

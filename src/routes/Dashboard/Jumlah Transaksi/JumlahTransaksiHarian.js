...
import ordersDataHarianYesterday from "../../../components/DataDummy/ordersHarianYesterday.json";
import ordersDataHarianToday from "../../../components/DataDummy/ordersHarianToday.json";
...
const JumlahTransaksiHarian = (props) => {
  ...
  const [amountToday, setAmountToday] = useState(
    ordersDataHarianToday[0][0].longValue
  );
  const [amountYesterday, setAmountYesterday] = useState(
    ordersDataHarianYesterday[0][0].longValue
  );
  ...
  useEffect(() => {
    setTransactionYesterday(
      amountYesterday === undefined ? 0 : Number(amountYesterday)
    );
  }, [amountYesterday]);
  // useEffect(() => {
  //   getAmountToday();
